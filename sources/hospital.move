module Hospital::hospital {
   ...
   //The administrator grants permission to the doctor.
    ...
    public fun new_patient(ctx:&mut TxContext,name:String){
        let patient=Patient{
            id:object::new(ctx),
    ...
    public fun pay_money(ctx:&mut TxContext,hospital:&mut Hospital,treatment: Treatment,patient:&mut Patient,clock: &Clock){
        assert!(!treatment.complete, TREATMENT_HAVE_COMPLETE);
